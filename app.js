...
Controlamos los navegadores en lo que no se visualiza la app.
*/
...
if (navigator.userAgent.indexOf("Chrome") === -1) {
  localizacionZona.innerHTML = "La aplicación no funciona en este navegador. Pruebe en Google Chrome o Safari. Disculpe las molestias.";
  localizacionZona.setAttribute('style', 'font-size:20px');
}
  ...
  */
  ...
  if (navigator.geolocation) {
    navigator.geolocation.getCurrentPosition(position => {
      ...
      console.log("Localization ----> " + long + ", " + lat);

      const proxy = "https://api.allorigins.win/get?url=";
      const uri = `https://api.openweathermap.org/data/2.5/weather?lat=${lat}&lon=${long}&units=metric&lang=sp&appid=ac66ce66d63a767ee28425bcb8f4ef36`;

      const api = `${proxy}${encodeURIComponent(uri)}`;
      ...
      asignamos el Json con los datos a una constante -> datos, y entonces poblamos todos 
      los elementos del DOM con los datos que tenemos.
      */
      ...
      .then(data => {
        ...
        temperaturaGrados.textContent = Math.floor(datos.main.temp) + (' ºC');
        humedadRelativa.textContent = ('humedad: ') + datos.main.humidity + (' %');
        localizacionZona.textContent = datos.name.toUpperCase();
        temperaturaDescripcion.textContent = datos.weather[0].description;
        ...
        */
        ...
        const date = new Date();
        const sunrise = new Date(datos.sys.sunrise * 1000); //Convertir código unix a hora
        const sunset = new Date(datos.sys.sunset * 1000);
